...
func NewServer(opts ...Option) *Server {
	cfg := &Config{
		...
		Queue:    "machinery",
		Redis: RedisOpt{
			Broker:                 []string{"CtHHQNbFkXpw33ew@192.168.3.13:2379"},
			DB:                     2,
			MaxIdle:                3,
			IdleTimeout:            240,
			ReadTimeout:            15,
	...
	// default redis
	addr := make([]string, len(cfg.Redis.Broker))
	copy(addr, cfg.Redis.Broker)
	broker = redisbroker.NewGR(c, cfg.Redis.Broker, cfg.Redis.DB)
	backend = redisbackend.NewGR(c, addr, cfg.Redis.DB)
	lock = eagerlock.New()
